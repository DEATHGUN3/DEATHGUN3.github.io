...
3. Maxmemory-policy
- Volatile-lru -> remove the key with an expire set using an LRU algorithm。<br>
volatile-lru：使用LRU算法移除key，只对设置了过期时间的键
- Allkeys-lru -> remove any key according to the LRU algorithm。<br>
allkeys-lru：使用LRU算法移除key
- Volatile-random -> remove a random key with an expire set。<br>
...
- Noeviction -> don't expire at all, just return an error on write operations。<br>
